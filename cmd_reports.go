...
func devicesListCmd(c *Command, client metalcloud.MetalCloudClient) (string, error) {
	...
	totalsRow := []interface{}{
		"",
		bold("TOTAL"),
		bold(fmt.Sprintf("%d", totalServers)),
		bold(fmt.Sprintf("%d", totalSwitches)),
		bold(fmt.Sprintf("%d", totalStorages)),
	}
