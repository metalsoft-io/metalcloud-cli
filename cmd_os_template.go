...
func updateTemplateFromCommand(obj metalcloud.OSTemplate, c *Command, client interfaces.MetalCloudClient, checkRequired bool) (*metalcloud.OSTemplate, error) {
	...
	if getBoolParam(c.Arguments["use_autogenerated_initial_password"]) {
		...
		if _, ok := getStringParamOk(c.Arguments["initial_user"]); ok {
			return nil, fmt.Errorf("--initial-user cannot be used with --use-autogenerated-initial-password")
		}
		...
		if v, ok := getStringParamOk(c.Arguments["initial_user"]); ok {
			...
			if checkRequired {
				return nil, fmt.Errorf("initial-user (or --use-autogenerated-initial-password) is required")
			}
		}
		...
		if v, ok := getStringParamOk(c.Arguments["initial_password"]); ok {
			...
			if checkRequired {
				return nil, fmt.Errorf("initial-password (or --use-autogenerated-initial-password) is required")
			}
		}
	}
