## metalcloud-cli network-device breakout get-config

Get the staged configuration of a breakout

### Synopsis

Display the breakout groups staged for the next deploy, together with the
...
```
metalcloud-cli network-device breakout get-config <network_device_id> <breakout_id> [flags]
```
