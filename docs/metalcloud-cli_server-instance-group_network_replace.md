## metalcloud-cli server-instance-group network replace

Create or replace the network configuration of a server instance group

### Synopsis

Create or replace (PUT) the network configuration of a server instance group.
...
```
metalcloud-cli server-instance-group network replace server_instance_group_id [flags]
```
