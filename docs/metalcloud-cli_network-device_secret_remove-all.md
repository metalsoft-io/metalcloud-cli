## metalcloud-cli network-device secret remove-all

Delete every secret of a network device

### Synopsis

Delete every secret stored for a network device.
...
```
metalcloud-cli network-device secret remove-all <network_device_id> [flags]
```
