## metalcloud-cli vm-pool cluster-host-containers

List the containers running on one cluster host

### Synopsis

List all containers deployed on a single cluster host of a VM pool.
...
```
metalcloud-cli vm-pool cluster-host-containers vm_pool_id host_id [flags]
```

### Options

```
  -h, --help   help for cluster-host-containers
```
