...
func templateCreateCmd(c *command.Command, client metalcloud.MetalCloudClient) (string, error) {
	...
	osTemplate := (*obj).(metalcloud.OSTemplate)

	retTemplate, err := client.OSTemplateCreate(osTemplate)
	if err != nil {
		return "", err
	}

	if command.GetBoolParam(c.Arguments["return_id"]) {
		return fmt.Sprintf("%d", retTemplate.VolumeTemplateID), nil
	}

	return "", err
}

func templateUpdateCmd(c *command.Command, client metalcloud.MetalCloudClient) (string, error) {
	obj, err := objects.ReadSingleObjectFromCommand(c, client)
	if err != nil {
		return "", err
	}
	...
	_, err = client.OSTemplateUpdate(osTemplate.VolumeTemplateID, osTemplate)
	if err != nil {
		return "", err
	}
...
func templateGetCmd(c *command.Command, client metalcloud.MetalCloudClient) (string, error) {
	...
	if !showCredentials {
		template.OSTemplateCredentials = nil
	}

	format := command.GetStringParam(c.Arguments["format"])
	ret, err := objects.RenderRawObject(*template, format, "OSTemplate")
	if err != nil {
		return "", err
	}
...
func createTemplateAssets(c *command.Command, client metalcloud.MetalCloudClient, repoTemplate RepoTemplate, templateName string) (string, error) {
	...
	if createError != nil {
		...
		_, updateError := templateUpdateCmd(&updateTemplateCommand, client)
