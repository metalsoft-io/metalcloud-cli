...
var FirmwareCatalogCmds = []command.Command{
	{
		...
		InitFunc: func(c *command.Command) {
			c.Arguments = map[string]interface{}{
				"config_format":             c.FlagSet.String("config-format", "yaml", colors.Red("(Required)")+" The format of the config file. Supported values are 'json' and 'yaml'."),
				"read_config_from_file":     c.FlagSet.String("f", command.NilDefaultStr, colors.Red("(Required)")+" The path to the config file."),
...
Lenovo example:
metalcloud-cli firmware-catalog create --config-format json --f .\example_lenovo.json --filter-server-types "M.8.8.2.v5"
...
HP example:
metalcloud-cli firmware-catalog create --config-format yaml --f .\example_hp_gen_11.yaml
...
func firmwareCatalogCreateCmd(c *command.Command, client metalcloud.MetalCloudClient) (string, error) {
	...
	if rawConfigFilePathValue, ok := command.GetStringParamOk(c.Arguments["read_config_from_file"]); !ok {
		return "", fmt.Errorf("the 'read_config_from_file' parameter must be specified when creating a firmware catalog")
	} else {
	...
	default:
		...
		return "", fmt.Errorf("invalid vendor '%s' found in the read_config_from_file. Supported vendors are %v", configFile.Vendor, validVendors)
	}
