...
func subnetOOBGetCmd(c *command.Command, client metalcloud.MetalCloudClient) (string, error) {
	...
	subnetOOB, err := client.SubnetOOBGet(id)
	if err != nil {
		return "", err
	}

	format := command.GetStringParam(c.Arguments["format"])
	ret, err := objects.RenderRawObject(*subnetOOB, format, "SubnetOOB")
	if err != nil {
		return "", err
	}
...
func subnetOOBCreateCmd(c *command.Command, client metalcloud.MetalCloudClient) (string, error) {
	...
	subnetOOB := (*obj).(metalcloud.SubnetOOB)

	createdSubnetOOB, err := client.SubnetOOBCreate(subnetOOB)
	if err != nil {
		return "", err
	}

	if command.GetBoolParam(c.Arguments["return_id"]) {
		return fmt.Sprintf("%d", createdSubnetOOB.SubnetOOBID), nil
	}
