...
func TestDriveArrayCreate(t *testing.T) {
	...
	for _, a := range errorArguments {
		...
		_, err := driveArrayCreateCmd(&command.Command{Arguments: a}, client)
...
func TestDriveArrayListCmd(t *testing.T) {
	...
	for _, a := range errorArguments {
		...
		_, err := driveArrayListCmd(&command.Command{Arguments: a}, client)
		Expect(err).NotTo(BeNil())
	}
...
func TestDriveArrayGetCmd(t *testing.T) {
	...
	cases := []command.CommandTestCase{
		{
			Name: "good1",
			Cmd:  command.MakeCommand(map[string]interface{}{"drive_array_id_or_label": 10}),
			Good: true,
		},
		{
			Name: "no id",
	...
	command.TestGetCommand(driveArrayGetCmd, cases, client, expectedFirstRow, t)
}
